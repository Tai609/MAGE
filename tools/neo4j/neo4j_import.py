...
# Keep only minimal aliases for backwards compatibility.
# For other type names, labels are generated dynamically from JSON type values.
NODE_TYPE_ALIASES = {
...
# Relationship type aliases for backward compatibility.
# If a type is not in this map, we generate Neo4j relationship type dynamically.
EDGE_TYPE_ALIASES = {
    "with_basicmaterial": "WITH_BASICMATERIAL",  # Renamed from HAS_COMPONENT
    "has_component": "WITH_BASICMATERIAL",
    "related_to": "RELATED_TO",
}


def _to_dynamic_label(type_value: str) -> str:
    """
    Convert JSON node type into a safe Neo4j label dynamically.
    Example:
      - "test_condition" -> "TestCondition"
      - "MEASURED_UNDER_CONDITION" -> "MeasuredUnderCondition"
    ...
    # Split on non-alnum chars, then build PascalCase.
    parts = [p for p in re.split(r"[^0-9A-Za-z]+", raw) if p]
    if not parts:
        return "UnknownNode"

    # Use normalized PascalCase so all-caps inputs are also canonicalized.
    label = "".join(p.lower().capitalize() for p in parts)
    if not label:
        return "UnknownNode"
    if label[0].isdigit():
        label = f"T{label}"
...
def _resolve_neo4j_label(node_type: str, raw_properties: dict) -> tuple[str, bool]:
    """
    Resolve final Neo4j label and catalyst flag.
    Dynamic mapping is default; catalyst override is preserved.
    """
    base_label = _to_dynamic_label(node_type)
    is_catalyst = False

    if base_label == "Chemical" and raw_properties.get("is_catalyst"):
        return "Catalyst", True

    if base_label == "Catalyst":
        is_catalyst = True

    return base_label, is_catalyst


def _to_dynamic_rel_type(edge_type_value: str) -> str:
    """
    Convert JSON edge type into a safe Neo4j relationship type dynamically.
    Example:
      - "measured_under_condition" -> "MEASURED_UNDER_CONDITION"
      - "TESTED_IN" -> "TESTED_IN"
      - "x-y/z" -> "X_Y_Z"
    """
    raw = str(edge_type_value or "").strip()
    if not raw:
        return "RELATED_TO"

    lowered = raw.lower()
    if lowered in EDGE_TYPE_ALIASES:
        return EDGE_TYPE_ALIASES[lowered]

    parts = [p for p in re.split(r"[^0-9A-Za-z]+", raw) if p]
    if not parts:
        return "RELATED_TO"

    rel_type = "_".join(p.upper() for p in parts)
    if rel_type[0].isdigit():
        rel_type = f"T_{rel_type}"
    return rel_type


def _resolve_neo4j_rel_type(edge_type_value: str) -> str:
    return _to_dynamic_rel_type(edge_type_value)
...
def import_graph_to_neo4j(
    ...
    for node in nodes_data:
        if isinstance(node, dict) and "id" in node:
            ...
            raw_properties = dict(node) # Access properties directly for flag check
            ...
            resolved_label, _ = _resolve_neo4j_label(node_type=node_type, raw_properties=raw_properties)
            if resolved_label == "Catalyst":
    ...
    with driver.session(**session_kwargs) as session:
        ...
        for node in nodes_data:
            ...
            node_type_str = str(node.get("type", "unknown")).lower()
            ...
            # Determine final Neo4j Label (dynamic by default)
            neo4j_label, is_catalyst_node = _resolve_neo4j_label(
            ...
            # Check if it's a shared/global type
            if neo4j_label in ["BasicMaterial", "Paper"]:
        ...
        for edge in edges_data:
            ...
            edge_type_raw = str(edge.get("type", "related_to"))
            edge_type_str = edge_type_raw.lower()
            neo4j_rel_type = _resolve_neo4j_rel_type(edge_type_raw)

            # Skip creating BasicMaterial relationships here as they are handled during node processing
            if neo4j_rel_type == "WITH_BASICMATERIAL":
                continue
...
def main():
    ...
    parser.add_argument("--neo4j_database", type=str, default=os.environ.get("NEO4J_DATABASE", "neo4j"), help="Neo4j target database name (default: neo4j or NEO4J_DATABASE env var).")
